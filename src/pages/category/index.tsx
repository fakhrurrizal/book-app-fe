...
import { Card, CircularProgress, Container, Grid, IconButton, Stack } from "@mui/material";
                ...
                <Grid container spacing={3}>
                    ...
                    {CategoryList && CategoryList?.length > 0 ? CategoryList
                        ...
                        ?.map((item, index) => (
                            <Grid item xs={12} md={4} key={index} className="animate__animated animate__fadeIn animate__delay-1s">
                                <Card onClick={() => { if (!manage) { router.push(`/category/${item?.id}`) } }} key={index} className="cursor-pointer bg-primary bg-opacity-15 hover:border-2 h-[220px] border-2 border-transparent hover:border-primary relative px-5 py-8 m-2 text-center flex flex-col rounded-3xl items-center justify-center">
                    ...
                    {manage &&
                        <Grid item xs={12} md={4} className="animate__animated animate__fadeIn">
                            <Card onClick={toggle} className=" cursor-pointer bg-primary bg-opacity-5 h-[220px] border-2 border-primary relative px-5 py-8 m-2 text-center flex flex-col rounded-3xl items-center justify-center">
